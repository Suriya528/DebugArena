...
export const CodingProblemDetails: React.FC<CodingProblemDetailsProps> = ({
  ...
  return (
    <div className={`space-y-5 text-xs font-sans ${className}`}>
      ...
      {/* 5. DEBUG INSTRUCTIONS SECTION (WHEN APPLICABLE - NO RAW CODE DISPLAY PER SECTION 12) */}
      {(parsed.bugClue || question.codingMode === 'debug' || question.type === 'debugging') && (
        <div className="rounded-2xl bg-amber-950/20 border border-amber-500/30 p-4 sm:p-5 shadow-lg shadow-black/20 space-y-2.5">
          <div className="flex items-center gap-2 text-amber-400 font-bold uppercase tracking-wider text-[11px]">
            <Bug className="w-4 h-4 text-amber-400 shrink-0" />
            <span>Debug Objective & Instructions</span>
          </div>
          <div className="text-slate-200 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap font-sans">
            {parsed.bugClue || 'The code editor on the right contains starter code with an intentional defect. Inspect the logic, correct the bug in the editor, and run test cases to verify your fix.'}
          </div>
        </div>
      )}

      {/* RAW ERROR CODE SECTION (ADMIN PREVIEW ONLY WHEN showErrorCode IS TRUE) */}
      {showErrorCode && parsed.errorCode && (
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="font-extrabold uppercase tracking-wider text-[11px] text-rose-300 block">
                  Starter Buggy Code Preview
                </span>
                <span className="text-[10px] text-slate-400">
                  Starter logic loaded into candidate editor
                </span>
              </div>
            </div>
          ...
          <div className="relative rounded-xl bg-slate-950 border border-slate-800/90 overflow-hidden">
            <div className="bg-slate-900/70 px-3 py-1.5 border-b border-slate-800/80 flex items-center justify-between text-[10px] font-mono text-slate-400">
              <span className="flex items-center gap-1.5">
                ...
                <span>starter_defective.{activeLanguage === 'python' ? 'py' : activeLanguage === 'javascript' ? 'js' : activeLanguage === 'cpp' ? 'cpp' : activeLanguage === 'c' ? 'c' : 'java'}</span>
              </span>
