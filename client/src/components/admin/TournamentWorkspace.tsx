...
export const TournamentWorkspace: React.FC<TournamentWorkspaceProps> = ({
  ...
  const adminControlUrl = (event as any)?.adminLink
    ? `${window.location.origin}${(event as any).adminLink}`
    : '';

  const [copiedAdminLink, setCopiedAdminLink] = useState(false);
  const [copiedEventCode, setCopiedEventCode] = useState(false);
  const [isRegeneratingAdminLink, setIsRegeneratingAdminLink] = useState(false);
  ...
  const handleCopyAdminLink = () => {
    if (!adminControlUrl) return;
    navigator.clipboard.writeText(adminControlUrl);
  ...
  const handleCopyEventCode = () => {
    ...
    setTimeout(() => setCopiedEventCode(false), 2500);
  };

  const handleRegenerateAdminLink = async () => {
    if (!event) return;
    if (
      !window.confirm(
        'Regenerate Admin Control Link? The old private admin URL will be permanently invalidated immediately. The participant link and event code will remain unchanged.'
      )
    ) {
      return;
    }
    try {
      setIsRegeneratingAdminLink(true);
      const res = await regenerateAdminLink(event._id);
      alert('Admin Control Link regenerated successfully! The old URL is now invalid.');
      fetchWorkspaceData();
    } catch (err: any) {
      alert(err.response?.data?.error || 'Failed to regenerate admin link');
    } finally {
      setIsRegeneratingAdminLink(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 animate-in fade-in duration-200 text-left">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl backdrop-blur-xl space-y-4">
        ...
        {/* Event Access & Security Panel */}
        <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3 shadow-sm">
          {/* Row 1: Participant Join URL */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/80">
            <div className="flex items-center gap-2 text-xs font-mono min-w-0 flex-1">
              <span className="px-2.5 py-1 rounded bg-indigo-500/20 text-indigo-300 font-bold text-[10px] uppercase shrink-0 border border-indigo-500/30">
                Participant Link
              </span>
              <span className="text-slate-200 truncate select-all font-semibold">
                {participantJoinUrl}
              </span>
            </div>
          ...
          {/* Row 2: Private Admin Control Link & Event Code */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3 text-xs font-mono min-w-0 flex-1 flex-wrap">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <span className="px-2.5 py-1 rounded bg-amber-500/20 text-amber-300 font-bold text-[10px] uppercase shrink-0 border border-amber-500/30">
                  Private Admin Link
                </span>
                <span className="text-slate-400 truncate select-all text-[11px]">
                  {adminControlUrl || 'Private Control URL Encrypted'}
                </span>
              </div>

              <div className="flex items-center gap-1.5 shrink-0 px-2.5 py-1 rounded-xl bg-slate-900 border border-slate-700/80 text-slate-300 text-xs">
                <span className="text-slate-500 text-[10px] font-bold uppercase">Event Key:</span>
                <span className="font-mono font-bold text-amber-400">{event?.code}</span>
                <button
                  onClick={handleCopyEventCode}
                  className="p-0.5 hover:text-white text-slate-400 cursor-pointer ml-1"
                  title="Copy Event Code"
                >
                  {copiedEventCode ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={handleRegenerateAdminLink}
                disabled={isRegeneratingAdminLink}
                className="px-3 py-1.5 rounded-xl text-xs font-mono font-bold flex items-center gap-1.5 bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 border border-rose-800/60 transition-all cursor-pointer disabled:opacity-50"
                title="Invalidate old link and generate a new cryptographically secure admin URL"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isRegeneratingAdminLink ? 'animate-spin' : ''}`} />
