    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
      <div className="w-full max-w-2xl rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6 space-y-6">
          <div className="relative p-8 rounded-3xl bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-2 border-amber-500/30 shadow-2xl text-center overflow-hidden">
            ...
            <div className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-200 mb-1">
              {name || username}
            </div>
            <div className="text-xs font-mono text-slate-500 mb-6">@{username}</div>

            <p className="text-xs text-slate-300 max-w-md mx-auto leading-relaxed mb-6">
              For demonstrating exceptional proficiency in competitive debugging, code optimization, and algorithmic problem-solving, achieving <strong className="text-amber-300">Rank #{rank}</strong> with an aggregate score of <strong className="text-amber-300">{score} points</strong>.
            </p>
            ...
            <div className="pt-6 border-t border-slate-800/80 flex items-center justify-between gap-4">
              <div className="text-left font-mono">
                ...
                <div className="text-xs font-bold text-slate-300">{certId}</div>
                <div className="text-[10px] text-slate-500 mt-1">Issued: {new Date().toLocaleDateString()}</div>
              </div>

              <div className="p-3 rounded-2xl bg-white flex flex-col items-center justify-center shadow-md">
              ...
              <div className="text-right font-mono">
                ...
                <div className="text-[9px] text-slate-500 truncate max-w-[140px]">{hash}</div>
                <div className="text-[10px] text-slate-400 mt-1">Tamper-Proof Ledger</div>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 py-3 border-t border-slate-800 bg-slate-950/60 flex items-center justify-between text-xs">
          <span className="text-slate-500 font-mono">Public Verification Endpoint: /api/certificates/verify/:id</span>
